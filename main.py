...
network = Network([8, 3, 8], alpha, reg_lambda)
network.fit(inputs, 10000)
network.test(inputs[1])

print(f"Weights input layer: {network.layers[0].get_thetas()}")
print(f"Bias input layer: {network.layers[0].get_bias()}")

print(f"Weights hidden layer: {network.layers[1].get_thetas()}")
print(f"Bias hidden layer: {network.layers[1].get_bias()}")

print(f"Hidden activations for {inputs[1]}: {network.layers[1].get_activations()}")